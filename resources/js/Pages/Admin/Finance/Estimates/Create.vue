  ...
  <AdminLayout>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Create Estimate</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400">Generate a new quotation and sync with Zoho Invoice.</p>
        </div>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03] space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Valid Date -->
            <div>
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Valid Until</label>
              <input v-model="form.valid_date" type="date" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 dark:border-gray-700 dark:text-white" required />
              <p v-if="form.errors.valid_date" class="mt-1 text-xs text-error-500">{{ form.errors.valid_date }}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea v-model="form.notes" rows="3" placeholder="e.g. Thanks for your business!" class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 dark:border-gray-700 dark:text-white"></textarea>
          ...
          <div class="flex flex-col md:flex-row md:items-end justify-between pt-4 border-t border-gray-100 dark:border-gray-800 gap-6">
            ...
            <div class="text-right space-y-2 w-full md:w-auto">
              ...
              <div class="flex justify-end gap-8 pt-2 border-t border-gray-100 dark:border-gray-800">
                <span class="text-sm font-bold text-gray-800 dark:text-gray-200 mt-1">Total Quotation Amount:</span>
        ...
        <div class="flex items-center justify-end gap-3">
          <Link href="/finance/estimates" class="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-white/[0.02]">Cancel</Link>
          <button
            type="submit"
          ...
          >
            ...
            {{ form.processing ? 'Creating…' : 'Create Estimate' }}
          </button>
        </div>
      </form>
    </div>
...
const form = useForm({
  customer_id: '',
  valid_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  notes: '',
  global_tax_id: '',
  shipping_charge: 0,
  line_items: [
...
const submit = () => {
  ...
  })).post(route('finance.estimates.store'))
}
</script>
